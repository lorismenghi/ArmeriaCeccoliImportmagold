...
*/
	...
	public function createOrUpdateConfigurableProduct($simpleProduct, $skuConfigurabile)
	{
		...
		foreach ($allSimpleProducts as $prod) {
			...
			if ($color !== null) $uniqueCombinations[] = $color;
			if ($tg !== null) $uniqueCombinations[] = $tg;
			if ($diametro !== null) $uniqueCombinations[] = $diametro;
			if ($calibro !== null) $uniqueCombinations[] = $calibro;

			$key = implode('-', $uniqueCombinations);

			//if (!isset($uniqueCombinations[$key])) {
				//$uniqueCombinations[$key] = $prod;
			//}

			if (!isset($uniqueCombinations[$key]) && !$this->isProductAlreadyAssociated($prod, $allSimpleProducts)) {
				$uniqueCombinations[$key] = $prod;
			}
	...
	public function isProductAlreadyAssociated($product, $associatedProducts)
	{
