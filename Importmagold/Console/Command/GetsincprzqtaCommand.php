...
class GetsincprzqtaCommand extends Command
{
    /**
     * @var State
     */
    ...
    {
        $this->setName('lm:importmagold:getsincprzqta')
            ->setDescription('Scarica e sinc da Mexal prezzi e quantità per il modulo Importmagold.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($isEnabled) {
            ...
            //$articoli_ricerca = $mexal->getApiArticoliRicercaResponse($output); // al suo interno fa ciclo chiamate e per ogni chiamata sinc articoli
			...
			try {
				while ($hasNext) {
				    ...
				    $output->writeln('<info>Inizio ciclo chiamate mexal con next:'.$next.'</info>');
					...
					if (file_exists($cacheFile) && !file_exists($flagFilePath)) {
				    	...
				    	$response = $mexal->getApiArticoliRicercaCall($next);
				    }
				    ...
				    if (isset($data['error'])) {
				        throw new \Exception("Errore nella risposta API: " . print_r($data['error'], true));
				    }
				    ...
				    $array_risposta = $mexal->getArrayRispostaArticoliRicerca($output, file_get_contents($cacheFile));
				    $articoli_ricerca_rielaborati = $mexal->getArrayRispostaArticoliRicercaRielaborati($output, $array_risposta);
				    // ora effettuo ciclo importazione
				    $n = $this->importProductMexalService->n;
					$this->importProductMexalService = new ImportProductMexalServiceCommand();
					$this->importProductMexalService->n = $n;
					$this->importProductMexalService->importProductQtaPrz($output, $articoli_ricerca_rielaborati);
					$primo_giro = false;
					$array_response = ['dati'=>[]];
            ...
            //$output->writeln('<comment>Scaricati un totale di '.count($articoli_ricerca_rielaborati).' articoli</comment>');
        } else {
